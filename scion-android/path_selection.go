...
func pathSelection(paths []snet.Path, pathAlgo int) snet.Path {
	...
	switch pathAlgo {
	case Shortest:
		fmt.Println("Path selection algorithm", "pathAlgo", "shortest")
		selectedPath, metric = pathAlgos[pathAlgo](paths)
	case MTU:
		fmt.Println("Path selection algorithm", "pathAlgo", "MTU")
		selectedPath, metric = pathAlgos[pathAlgo](paths)
	default:
	...
	fmt.Println("Path selection algorithm choice", "path", fmt.Sprintf("%s", selectedPath), "score", metric)
	return selectedPath
}
